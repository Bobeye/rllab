...
n_sub_iter=pd.read_csv("n_sub_iter_vA_swimmer_sn2.csv")
rewards_snapshot=pd.read_csv("rewards_snapshot_swimmer_vA_sn2.csv")
rewards_subiter=pd.read_csv("rewards_subiter_swimmer_vA_sn2.csv")
variance_sgd=pd.read_csv("variance_sgd_vA_swimmer_sn2.csv")
variance_svrg=pd.read_csv("variance_svrg_vA_swimmer_sn2.csv")
importance_weights=pd.read_csv("importance_weights_vA_swimmer_sn2.csv")
gpomdp_rewards=pd.read_csv("rewards_snapshot_GPOMDP_swimmer_sgd.csv")

alfa = pd.read_csv("alfa_t_sn2.csv")
diff_lr = pd.read_csv("diff_lr_sn2.csv")
...
for col_name_s,col_name_si,col_name_nsi in zip(rewards_snapshot,rewards_subiter,n_sub_iter):    
    ...
    for i,k,s in zip(first[0:-1],ranges,range(len(ranges))):
        traj_rewards.append(list(np.repeat(0,100)))
...
for col_name_s,col_name_si,col_name_nsi in zip(rewards_snapshot,rewards_subiter,n_sub_iter):    
    ...
    for i,k,s in zip(first[0:-1],ranges,range(len(ranges))):
        ...
        a.append(np.mean(np.array(rewards_snapshot[col_name_s][s][1:-1].split(),dtype = np.float32)))
        for j in range(np.int(k)):
            traj_rewards.append(list(rewards_subiter[col_name_si][i+j][1:-1].split()))
            a.append(np.mean(np.array(rewards_subiter[col_name_si][i+j][1:-1].split(),dtype = np.float64)))
    if(len(ranges)<len(rewards_snapshot[col_name_s][rewards_snapshot[col_name_s]==rewards_snapshot[col_name_s]])):
        traj_rewards.append(list(rewards_snapshot[col_name_s][np.int(s+1)][1:-1].split()))
    traj_rewards=flatten(traj_rewards)
    avg_traj_rewards.append(traj_rewards)
