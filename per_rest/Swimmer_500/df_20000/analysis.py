...
n_sub_iter=pd.read_csv("n_sub_iter_vA_swimmer_sn14.csv")
rewards_snapshot=pd.read_csv("rewards_snapshot_swimmer_vA_sn14.csv")
rewards_subiter=pd.read_csv("rewards_subiter_swimmer_vA_sn14.csv")
variance_sgd=pd.read_csv("variance_sgd_vA_swimmer_sn14.csv")
variance_svrg=pd.read_csv("variance_svrg_vA_swimmer_sn14.csv")
importance_weights=pd.read_csv("importance_weights_vA_swimmer_sn14.csv")
...
avg_traj_rewards2=np.asarray(np.mean(np.matrix(avg_traj_rewards[1:],dtype=np.float64),axis=0)).flatten()
...
for col in gpomdp_rewards:
    ...
    rew=list()
    for i in x: 
        rew.append(list(i[1:-1].split())[:10])
