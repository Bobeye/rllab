...
for k in range(10):
    ...
    while j<s_tot-N:
        ...
        while j<s_tot-M:
            ...
            stp = f_update[n_sub](g[0],g[1],g[2],g[3],g[4])
            print("step:",stp)
    ...
    self_weights_data["selfWeights"+str(k)] = self_norm_w
...
self_weights_data = pd.DataFrame(dict([ (k,pd.Series(v)) for k,v in self_weights_data.items() ]))

rewards_subiter_data.to_csv("rewards_subiter_v2_V_sn.csv",index=False)
rewards_snapshot_data.to_csv("rewards_snapshot_v2_V_sn.csv",index=False)
n_sub_iter_data.to_csv("n_sub_iter_v2_V_sn.csv",index=False)
variance_sgd_data.to_csv("variance_sgd_v2_V_sn.csv",index=False)
variance_svrg_data.to_csv("variance_svrg_v2_V_sn.csv",index=False)
importance_weights_data.to_csv("importance_weights_v2_V_sn.csv",index=False)
self_weights_data.to_csv("self_weights_v2_V_sn.csv",index=False)

alla.to_csv("GPOMDP_SVRG_adaptive_m06_ver2_V_sn.csv",index=False)