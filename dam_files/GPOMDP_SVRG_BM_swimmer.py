...
importance_weights_bs = dist.likelihood_ratio_sym_1traj_GPOMDP(actions_var,snap_dist_info_vars,dist_info_vars)
...
f_importance_weights_bs = theano.function(
    inputs = [observations_var, actions_var],
    outputs = importance_weights_bs
)
    ...
    while j<s_tot-N:
        ...
        for kx in range(N):
            ...
            for yk in range(len(x)):
                y = x[yk]
                z.append(y*t)
                za.append(t*(y - (path_baseline[yk]-discount*path_baseline[yk+1])))
                t*=discount
            return_so_far = 0    
...
#        print("step snapshot:", stp_snp)
        ...
        baseline_snap.set_param_values(baseline.get_param_values())
        ...
        while j<s_tot-M:
            ...
            for kx in range(M):
                x = sub_d_rewards[kx]
                path = dict(
                        observations=np.array(sub_observations[kx]),
                        actions=np.array(sub_actions[kx]),
                        rewards=np.array(x),
                        )
                z=list()
                z_rew = list()
                ...
                path_baseline = baseline.predict(path)
                path_baseline = np.append(path_baseline,0)      
                path_baseline_sn = baseline_snap.predict(path)
                path_baseline_sn = np.append(path_baseline_sn,0)  
                for yk in range(len(x)):
                    y = x[yk]
                    z.append(y*t)
                    za.append(t*(y - (path_baseline[yk]-discount*path_baseline[yk+1])))
                    za_snap.append(t*(y - (path_baseline_sn[yk]-discount*path_baseline_sn[yk+1])))
                    t*=discount
                return_so_far = 0
                for t in range(len(x) - 1, -1, -1):
                    return_so_far = x[t] + discount * return_so_far
                ...
                temp.append(np.array(z))
                tempa.append(np.array(za))
                tempa_sn.append(np.array(za_snap))
                path["advantages"] = za
                path["returns"] = z_rew
                p_4si.append(path)
                
#            for i_N in range(N):
#                im_bs = f_importance_weights_bs(p_4b[i_N]['observations'],p_4b[i_N]['actions'])
#                p_4si.append(p_4b[i_N])
#                p_4si[-1]['returns'] = p_4si[-1]['returns']*im_bs
#                return_so_far = 0
#                z_rew = list()
#                x = p_4si[-1]['rewards']
#                for t in range(len(x) - 1, -1, -1):
...
#            baseline.fit(p_4si)
            ...
            s_g_is = f_imp_SVRG(sub_observations[0], sub_actions[0], sub_d_rewards_sn[0],iw_var)
            s_g_fv_is = [unpack(s_g_is)]
            w_cum=np.sum(dis_iw(iw_var))
            for ob,ac,rw,rw_sn in zip(sub_observations[1:],sub_actions[1:],sub_d_rewards[1:],sub_d_rewards_sn[1:]):
                ...
                s_g_is_sgd = f_imp_SVRG(ob, ac, rw_sn,iw_var)
...
rewards_subiter_data.to_csv("rewards_subiter_ba.csv",index=False)
rewards_snapshot_data.to_csv("rewards_snapshot_ba.csv",index=False)
n_sub_iter_data.to_csv("n_sub_iter_ba.csv",index=False)
variance_sgd_data.to_csv("variance_sgd_ba.csv",index=False)
variance_svrg_data.to_csv("variance_svrg_ba.csv",index=False)
importance_weights_data.to_csv("importance_weights_ba.csv",index=False)
ar_data.to_csv("ar_va_b_ba.csv",index=False)
all_policy_param_data.to_csv("param_policy_ba.csv",index=False)