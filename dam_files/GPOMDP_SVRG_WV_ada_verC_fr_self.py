    ...
    while j<s_tot-N:
        ...
        while j<s_tot-M:
            ...
            for ob,ac,rw in zip(observations[1:],actions[1:],d_rewards[1:]):
                ...
                importance_weights.append(np.mean(iw_var))
                w_cum+=np.sum(dis_iw(iw_var))
            s_g_fg_c  = [x/w_cum for x in s_g_fg_c]
            var_4_fg = np.cov(s_g_fv_fg,rowvar=False)
            var_fg = var_4_fg/(w_cum)
...
#            print('var term neg ',str(np.trace(var_term_neg)))
#            print('var cov ',str(np.trace(cov)))
