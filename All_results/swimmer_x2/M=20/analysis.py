...
gpomdp_rewards=pd.read_csv("rewards_snapshot_GPOMDP_mountain_sgd.csv")
...
for col_name_s,col_name_si,col_name_nsi in zip(rewards_snapshot,rewards_subiter,n_sub_iter):    
    ...
    for i,k,s in zip(first[0:-1],ranges,range(len(ranges))):
        ...
        for j in range(np.int(k)):
            traj_rewards.append(list(rewards_subiter[col_name_si][i+j][1:-1].split())[:20])
...
for col in gpomdp_rewards:
    ...
    rew=list()
    for i in x: 
        rew.append(list(i[1:-1].split())[:20])
    reward.append(flatten(rew[:500])+flatten(rew[500:]))
