...
for k in range(10):
    if (load_policy):
        snap_policy.set_param_values(np.loadtxt('policy_mountain_novar.txt'), trainable=True)
    ...
    while j<s_tot-N:
        ...
        while j<s_tot-M:
            ...
            for ob,ac,rw in zip(observations[1:],actions[1:],d_rewards[1:]):
                ...
                importance_weights.append(np.mean(iw_var))
                iw_cum += np.max(iw_var)
            print("iw cum: " + str(iw_cum)) 
            s_g_fg_c  = [x/iw_cum for x in s_g_fg_c] #len(paths)
            ...
            print("var fg: " + str(np.trace(var_fg)))
            print("var is: " + str(np.trace(var_term_neg)))
...
rewards_subiter_data.to_csv("rewards_subiter_v3.csv",index=False)
rewards_snapshot_data.to_csv("rewards_snapshot_v3.csv",index=False)
n_sub_iter_data.to_csv("n_sub_iter_v3.csv",index=False)
variance_sgd_data.to_csv("variance_sgd_v3.csv",index=False)
variance_svrg_data.to_csv("variance_svrg_v3.csv",index=False)
importance_weights_data.to_csv("importance_weights_v3.csv",index=False)

alla.to_csv("GPOMDP_SVRG_adaptive_m06_ver3.csv",index=False)
